...
class MAEHead(BaseHead):
    ...
    def __init__(self, 
        ...
        # transformer decoder
        self.transformer_decoder = Transformer(**transformer_decoder)
    ...
    def forward(self, x, idx=None, mask_info=None):
        """
        [params] input_x (torch.Tensor: (n, t, p, d))
        [params] idx (torch.Tensor: (n, t)) idx of each head to utilize
        [params] recover_idx (torch.Tensor: (n, p)) idx of tokens before masking
        [returns] pred_x (torch.Tensor: (n, t, c, h, w)) reconstruction of original x
        """
        ...
        if mask_info is None:
            ...
            recover_idx = mask_info['recover_idx'] 

        x = rearrange(x, 'n t p d -> (n t) p d')

        # Transformer
        x = self.embed_linear(x)
        x = self.nekot_ksam(x, recover_idx) # reverse mask_token operation
        x = x + self.pos_embed
        x, _ = self.transformer_decoder(x)
        x = self.norm(x)
