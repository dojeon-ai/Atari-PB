...
class SiamMAENeck(BaseNeck):
    ...
    def forward(self, x, idx=None, is_target=False):
        """
        [params] x (torch.Tensor: (n, t, c, h, w))
        [params] idx (torch.Tensor: (n, t)) idx of the spatial embedding
        [params] is_target (bool) mask on only when this is true
        ...
        if is_target:
            x, recover_idx = self.mask_token(x, self.mask_ratio) # mask
