...
python run_pretrain.py \
    ...
    --overrides ++model.head.type=spr_idm_head \
    --overrides ++trainer.type='spr_idm' \
    --overrides ++trainer.tau_scheduler='{initial_value:0.99,final_value:0.999,max_step:-1}' \
    --overrides ++trainer.target_update_every=1 \
    --overrides ++trainer.spr_weight=2.0 \
    --overrides ++trainer.idm_weight=1.0 \
